...
$setareLivrare = backend\models\SetariLivrare::findOne(['restaurant' => 10]);
unset($basket->basketItems[$setareLivrare->produs0->id]);

$total = 0.00;
foreach ($basket->basketItems as $bi) {
    if ($bi->denumire != 'Cost livrare') {
        $total = $total + $bi->pret * $bi->cantitate;
    }
}


if ($setareLivrare->comanda_minima > $total) {
    $total += $setareLivrare->produs0->pret_curent;
    $pr = $setareLivrare->produs0;
    $basket->basketItems[$pr->id] = new \frontend\models\BasketItem(['idProdus' => $pr->id, 'cantitate' => 1, 'pret' => $pr->pret_curent, 'denumire' => 'Cost livrare']);
}
